...
def extract_json(text: str):
    """
    从模型输出中提取 JSON。
    模型正常情况下会直接返回 JSON，此函数用于去掉可能出现的额外文本。
    """
    text = text.strip()
...
def main():
    parser = argparse.ArgumentParser(
        description="使用 Qwen3-VL 识别交通标志类型"
    )

    parser.add_argument(
        "--image",
        type=str,
        default=r"/scrinvme/huilin/tp/DA5324655_20251006155244700.jpg",
        help="待识别图像",
    )
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="traffic_sign_result.json",
        help="JSON 结果保存",
    )
    ...
    # 转换为 file:///... URI，供 qwen_vl_utils 读取本地图像
    ...
    try:
        ...
        if result.get("has_traffic_sign") is True:
            print(f"检测到标志数量：{len(result.get('objects', []))}")

            for obj in result.get("objects", []):
                print(
                    f"[{obj.get('id')}] "
                    f"{obj.get('chinese_name')} | "
                    f"{obj.get('category')} | "
                    f"{obj.get('specific_type')} | "
                    f"confidence={obj.get('confidence')}"
                )

        elif result.get("has_traffic_sign") is False:
            print("图像中未检测到交通标志牌。")
